# =============================================================
# Production environment  --  Vault namespace provisioning
...
variable "vault_address" {
  type        = string
  description = "Vault cluster URL for the prod environment"
}
...
locals {
  ...
  # Base redirect URIs for OIDC login (prod URLs)
  oidc_redirect_uris = [
...
module "payments_namespace" {
  ...
  oidc_ns_admin_group_id   = "<payments-prod-aad-group-object-id>"  # replace with real AAD group OID
}
...
module "platform_namespace" {
  ...
  oidc_client_secret       = var.oidc_client_secret
  oidc_redirect_uris       = local.oidc_redirect_uris
  oidc_ns_admin_group_id   = "<platform-prod-aad-group-object-id>"  # replace with real AAD group OID
}
